(function() {
	...
	var Player = Mario.Player = function(pos) {
		...
		this.targetPos = [];
		this.flying = false;
		this.flyButtonReleased = true;
		// Flight meter: seconds of flight available (drains while holding fly)
		this.flyMeterMax = 3.0; // 3 seconds of flight
		this.flyMeter = this.flyMeterMax;
		this.flyAvailable = true; // whether player may start flying
		// Cooldown after meter fully used
		this.cooldownActive = false;
		this.cooldownTimer = 0;
		this.cooldownMax = 10.0; // 10 seconds cooldown
		// Whether the flight timer is currently running (started when player begins flying)
		this.flightTimerRunning = false;
		// Flash when cooldown ends
		this.flashActive = false;
		this.flashTimer = 0;
		this.flashTicks = 0; // number of toggle ticks remaining (on/off counts)
		this.flashInterval = 0.15; // seconds per toggle
	...
	Player.prototype.fly = function() {
			// Only start flying if allowed and there's meter left
			if (!this.flyAvailable || this.flyMeter <= 0) return;
			// If this is the moment flight is started, start the flight timer
	...
	Player.prototype.noFly = function() {
	 		// Stop flying. If the flight timer was running, starting cooldown immediately
	 		if (this.flying && this.flightTimerRunning) {
	 			this.flying = false;
	 			this.flightTimerRunning = false;
	 			this.flyAvailable = false;
  ...
  Player.prototype.setAnimation = function() {
	...
	Player.prototype.update = function(dt, vX) {
		...
		// Flight timer handling
		if (this.flying && this.flightTimerRunning) {
			// advance flight timer while flying
			this.flyMeter -= dt;
			if (this.flyMeter > 0) {
				this.vel[1] = -2.5;
			} else {
				// flight time exhausted => end flight and start cooldown
				this.flying = false;
				this.flightTimerRunning = false;
				this.flyAvailable = false;
				this.cooldownActive = true;
			...
			// Not currently flying: if cooldown active, advance cooldown
			if (this.cooldownActive) {
				this.cooldownTimer += dt;
				if (this.cooldownTimer >= this.cooldownMax) {
					// cooldown finished: restore meter and allow flying again
					this.cooldownActive = false;
					this.flyAvailable = true;
					this.flightTimerRunning = false;
					this.flyMeter = this.flyMeterMax;
					// Start flash sequence: 3 blue flashes (on/off ticks = 6)
					this.flashActive = true;
					this.flashTimer = 0;
					this.flashTicks = 6; // 3 full flashes (on+off each)
					this.flashVisible = false;
				}
			}
		}

		// Handle flash ticks (toggle blue flash visibility)
		if (this.flashActive) {
			this.flashTimer += dt;
